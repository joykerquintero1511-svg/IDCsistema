<?php
// --- PEGAR ESTO JUSTO DEBAJO DE TU INCLUDE DE CONEXIÓN --
...
// 1. Valores por defecto (Modo Crear una tarea nueva)
$titulo_tarea = "";
$tema = "";
$fecha_limite = "";
$form_action = "procesar_asignacion.php"; // El archivo que ya usas para guardar tareas
$boton_texto = "Publicar en el Aula";
$es_edicion = false;

// 2. Detectar si el profesor hizo clic en "Editar"
if (isset($_GET['id_edit'])) {
    $id_edit = $_GET['id_edit'];
    
    // Buscamos los datos de esa tarea específica en la tabla 'asignacion'
    $query_edit = "SELECT * FROM asignacion WHERE id_asignacion = $id_edit";
    $res_edit = mysqli_query($conexion, $query_edit);

    if ($res_edit && mysqli_num_rows($res_edit) == 1) {
        $tarea_edit = mysqli_fetch_assoc($res_edit);
        
        // Rellenamos las variables con los datos de la base de datos
        $titulo_tarea = $tarea_edit['titulo_tarea'];
        $tema = $tarea_edit['tema'];
        $fecha_limite = $tarea_edit['fecha_limite'];
        
        // Cambiamos el destino del formulario y el texto del botón
        $form_action = "procesar_editar.php";
                    ...
                    <?php if ($result_tareas && mysqli_num_rows($result_tareas) > 0): ?>
                        <?php while($tarea = mysqli_fetch_assoc($result_tareas)): ?>
                            <li class="task-item">
                                ...
                                <a href="crear_asignacion.php?id_edit=<?php echo $tarea['id_asignacion']; ?>" class="btn-action btn-edit">Editar</a>
                                <a href="procesar_eliminar.php?id=<?php echo $tarea['id_asignacion']; ?>" class="btn-action btn-delete" onclick="return confirm('¿Seguro que quieres eliminar esta asignación?');">Eliminar</a>
                            </li>
                        <?php endwhile; ?>
                    <?php else: ?>
