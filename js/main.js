...
// ---- INTERACTIVIDAD CON SCROLL AUTOMÁTICO PARA EFB V2 ----
document.addEventListener("DOMContentLoaded", function () {
    const programItems = document.querySelectorAll(".program-item");
    const descriptionText = document.getElementById("description-text");
    const descriptionBox = document.getElementById("program-description-box");

    programItems.forEach(item => {
        item.addEventListener("click", function () {
            // 1. Quitar la clase activa de todos los elementos
            programItems.forEach(i => i.classList.remove("is-active"));

            // 2. Añadir clase activa al elemento cliqueado
            this.classList.add("is-active");

            // 3. Obtener la descripción guardada en data-desc
            const newDescription = this.getAttribute("data-desc");

            // 4. Animación suave para cambiar el texto
            descriptionBox.style.opacity = 0;

            setTimeout(() => {
                descriptionText.innerHTML = newDescription;
                descriptionBox.classList.add("has-content");
                descriptionBox.style.opacity = 1;

                // 5. ¡LA MAGIA! Desplaza la pantalla suavemente hacia la caja de descripción
